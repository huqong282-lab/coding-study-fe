...
import { requestPasswordReset, verifyForgotPassword } from '../../services/authServices'
...
function ForgotPasswordOtpScreen() {
  ...
  async function handleVerify() {
    ...
    try {
      const response = await verifyForgotPassword({ email, otp: otp.join('') })
      setStatus('success')
      setMessage(response || 'Kode OTP berhasil diverifikasi.')
      setIsOtpVerified(true)
    } catch (error) {
      setStatus('error')
      setMessage(error instanceof ApiError ? error.message : 'Verifikasi OTP gagal.')
    }
  }
  ...
  async function handleResend() {
    ...
    try {
      ...
      setStatus('success')
      setMessage(response || 'Kode OTP baru telah dikirim ke email kamu.')
      setOtp(Array(OTP_LENGTH).fill(''))
      focusInput(0)
    } catch (error) {
      setStatus('error')
      setMessage(error instanceof ApiError ? error.message : 'Gagal mengirim ulang OTP.')
    }
  }
        ...
        <section className="relative w-full max-w-lg" aria-labelledby="otp-title">
          <div className="rounded-3xl border border-white/10 bg-slate-950 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
            <div className="mb-8 text-center">
              ...
              <p className="mt-5 text-xs font-semibold uppercase tracking-[0.32em] text-violet-300/80">Reset akses akun</p>
              <h1 id="otp-title" className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl">Verifikasi OTP</h1>
              <p className="mt-3 text-sm leading-6 text-slate-400 sm:text-base">
                Kode OTP reset telah dikirim ke <span className="font-semibold text-slate-200">{email}</span>. Masukkan kode 6 digit untuk melanjutkan reset password.
              </p>
            </div>
            ...
            <div className="grid gap-3" aria-label="Input kode OTP">
              ...
              <p className="text-center text-sm text-slate-400">
                Kirim ulang OTP tersedia dalam <span className="font-semibold text-violet-300">{formattedTime}</span>
              </p>
            </div>
