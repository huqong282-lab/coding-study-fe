...
- [x] 🎨 Setup _routing_ struktur aplikasi dasar (React Router DOM)
...
- [x] 🗄️ Tabel `otp_requests` (id, email, otp_code, expires_at, is_used)
...
- [x] 🔧 Setup NodeMailer / Integrasi provider email (SendGrid/Resend)
- [x] 🔧 Service `EmailSender` untuk mengirim email transaksional
- [x] 🔧 `POST /api/auth/forgot-password` — Generate OTP 6 digit, simpan ke `otp_requests`, kirim via email
- [x] 🔧 `POST /api/auth/verify-otp` — Verifikasi kode OTP dan masa berlakunya
- [x] 🔧 `POST /api/auth/reset-password` — Mengubah password user pasca verifikasi OTP

### 2.4 Role-Based Access Control (RBAC)

- [ ] 🗄️ Tabel `roles` (id, name, description, timestamps)
- [ ] 🗄️ Tabel `permissions` (id, name, description, timestamps)
- [ ] 🗄️ Tabel `role_permissions` (role_id, permission_id)
...
- [ ] 🎨 Implementasi Drag & Drop untuk mengatur _urutan_ modul
...
- [ ] 🎨 Komponen Testimonial Card untuk menampilkan _list review_ di halaman katalog.
